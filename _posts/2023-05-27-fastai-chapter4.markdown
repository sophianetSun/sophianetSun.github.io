...
데이터는 건드리지 않은 채 텐서의 모양만 바꿔주는 역할

28. 신경망의 편향 파라미터란 무엇인가요? 왜 필요할까요?

가중치를 곱하는 계산이 수행될 때 값이 0인 경우에는 결과값이 0 이 되어 변화가 없게 된다. 그래서 이를 피하기 위해 편향 bias도 가중치(weight)와 더불어 파라미터에 포함된다.
...
계산된 손실의 그레이디언트를 구함. 이미 계산된 그레이디언트가 있는 경우 더함.

31. 그레이디언트를 0으로 만들어야만 하는 이유는 무엇인가요?

이미 계산된 그레이디언트가 있는 경우 더하게 되어, 잘못된 값이 구해짐.

32. Learner로 넘겨줘야 하는 정보에는 무엇이 있나요?

DataLoaders(학습, 평가 데이터), 모델, 최적화 함수, 손실함수, [평가지표]
...
ReLU는 신경망에 비선형성을 추가해주기 위한 함수인데, 단순히 모든 음수값을 0으로 바꿔주는 함수

![relu](/assets/ml/relu.png)

35. 활성화 함수란 무엇인가요?

비선형성을 가진 ReLU와 같은 함수

36. F.relu와 nn.ReLU의 차이는 무엇인가요?

F.relu는 파이토치에서 제공 하는 함수, nn.ReLU는 같은 역할을 하는 파이토치의 클래스이다.

37. 만능 근사 정리는 비선형성 하나만으로도 모든 함수를 거의 근사할 수 있음을 보여줍니다. 그렇다면 왜 보통 비선형성이 하나 이상 필요할까요?

선형계층(덧셈이나 곱셈)을 아무리 많이 덧붙인다고 해도 그 구조가 변하지 않기 때문에 비선형성이 필요하다.
